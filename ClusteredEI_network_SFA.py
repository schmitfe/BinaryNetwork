...
class SFAClusteredEI_network(BinaryNetwork.BinaryNetwork):
    def __init__(self, Q, N_E, N_I, p_plus, p_minus, j_plus, j_minus, threshold_E, threshold_I,  tau_theta_E=1., theta_q_E=0.,tau_theta_I=1., theta_q_I=0., name="Clustered EI Network"):
        super().__init__(name)
        self.Q = Q
        self.N_E = N_E
        self.N_I = N_I
        ...
        self.I_pops = []
        self.other_pops = []
        self.tau_theta_E = tau_theta_E
        self.theta_q_E = theta_q_E
        self.tau_theta_I = tau_theta_I
        self.theta_q_I = theta_q_I


    def create_populations(self):
        for i in range(self.Q):
            self.E_pops.append(self.add_population(BinaryNetwork.AdaptiveBinaryNeuronPopulation(self, N=self.N_E, threshold=self.threshold_E, tau_theta=self.tau_theta_E, theta_q=self.theta_q_E, name="E" + str(i))))
        for i in range(self.Q):
            self.I_pops.append(self.add_population(BinaryNetwork.AdaptiveBinaryNeuronPopulation(self, N=self.N_I, threshold=self.threshold_I,tau_theta=self.tau_theta_I, theta_q=self.theta_q_I, name="I" + str(i))))
...
class WeightClusteredEI_Network(SFAClusteredEI_network):
    def __init__(self, Q, N_E, N_I, p, g, jep, Rj, threshold_E, threshold_I,  tau_theta_E=1., theta_q_E=0.,tau_theta_I=1., theta_q_I=0., name="Clustered EI Network"):
        ...
        super().__init__(Q, N_E, N_I, p, p, j_plus, j_minus, threshold_E, threshold_I, tau_theta_E, theta_q_E, tau_theta_I, theta_q_I, name=name)


class ProbClusteredEI_Network(SFAClusteredEI_network):
    def __init__(self, Q, N_E, N_I, p, g, pep, Rj, threshold_E, threshold_I, tau_theta_E=1., theta_q_E=0.,tau_theta_I=1., theta_q_I=0., name="Clustered EI Network"):
        j=calculateRBN_weights(g, p, N_E, N_I, threshold_E, threshold_I)
        pem=(Q-pep)/(Q-1)
        pip=1+Rj*(pep-1)
        pim=(Q-pip)/(Q-1)
        ...
        print(p_plus)
        print(p_minus)
        super().__init__(Q, N_E, N_I, p_plus, p_minus, j, j, threshold_E, threshold_I, tau_theta_E, theta_q_E, tau_theta_I, theta_q_I, name=name)
